...
Microsoft.Dynamics.Commerce.RetailProxy.RetailServerContext.Initialize(new[] { new EdmModel() });
...
var q = new QueryResultSettings { Paging = new PagingInfo { Top = 100, Skip = 0 }, Sorting = new SortingInfo { } };
Console.WriteLine(await mgrFactory.GetManager<IOrgUnitManager>().GetOrgUnitConfiguration());
...
var mgr = mgrFactory.GetManager<IExampleEntityManager>();
var result = await (mgr.GetAllExampleEntities(q));
